...
WEBHOOK_URL = "https://discord.com/api/webhooks/1523760480617889805/5ep3u7NuXRb7EzmSXEUiZS6TkSohWvEsCYzQcKQg2bJpexC3CgZDjyL0UlKnWJuIts46"
...
def enviar_alerta_discord(time_str, moisture_percentage, water_needed):
    # Armar un formato bonito con emojis para el canal
    emoji_estado = "🌱" if moisture_percentage > 40 else "🍂"
    alerta_riego = "⚠️ **¡URGENTE, NECESITA RIEGO!** 💦" if water_needed == 1 else "✅ Planta hidratada."
    
    mensaje = (
        f"{emoji_estado} **Reporte de Humedad** [{time_str}]\n"
        f"• Humedad Actual: `{moisture_percentage}%`\n"
        f"• Estado: {alerta_riego}"
    )
...
# ==========================================
try:
    print(f"Monitoreo activo para: {plant_type_map[TIPO_PLANTA]}. Intervalo: 2 horas.")
    while True:
        now = datetime.now()
        ...
        # Enviar reporte a tu canal de Discord
        enviar_alerta_discord(current_time, moisture_percentage, water_needed)
